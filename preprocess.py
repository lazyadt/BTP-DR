...
if __name__ == '__main__':
    ...
    if verbose:
        draw_patches(patches[:100], H=patchSize, W=patchSize)
        draw_patches(Xcont[:100], H=patchSize, W=patchSize)
        draw_patches(xZCAWhite[:100], H=patchSize, W=patchSize)
...
#    reader = csv.reader(open(constants.labels_file, mode='r'))
 #   next(reader)  # skip header row
  #  labels_dict = {row[0]:int(row[1]) for row in reader}
   # label = labels_dict.values()

    with h5py.File(constants.train_features_file, 'r') as fr:
        #image = fr['X_train'][:]
        label = fr['X'][:]
    #    label = fr['y'][:] 
    
    with h5py.File(constants.train_features_scaled_file, 'r') as fs:    
        image= fs['X'][:]
